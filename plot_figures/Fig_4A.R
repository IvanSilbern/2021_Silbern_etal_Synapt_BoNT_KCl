local({
  ...
  reg_group <- ph_cand$Regulation_group
  levels(reg_group) <-c("primary Ca-dependent", "Ca-opposing", "Ca-synergistic", "Ca-undefined", "not-regulated")
  ph_cand[, Regulation_group := reg_group]
  
  # do not specify Ca-effect
  ph_cand[, Regulation_group2 := "not-affected"]
  ph_cand[abs(log2FC.CaEGTA) > log2(1.2) & abs(log2FC.BoNT) < log2(1.2), Regulation_group2 := "primary Ca-dependent"]
  ph_cand[abs(log2FC.BoNT) > log2(1.2), Regulation_group2 := "SV-cycling-dependent"]
  ph_cand[, Regulation_group2 := factor(Regulation_group2, c("primary Ca-dependent", "SV-cycling-dependent", "not-affected"))]
  ...
  pdf("Figures\\Fig_4A\\Fig_4A_mono.pdf", width = 10 , height = 7)
  
  g <- ggplot(ph_cand, aes(x = log2FC.BoNT, y = log2FC.CaEGTA_BoNT, color = Regulation_group))
  g <- g + geom_point(alpha = 0.6)
  g <- g + scale_color_manual(values = c("#ffa555", "#30a8ff", "#3071ff", "#30d9ff", "grey"))
  g <- g + geom_abline(slope = lm_mod$coefficients[2], intercept = lm_mod$coefficients[1], color = "steelblue", linetype = "dashed")
  g <- g + xlab("log2 (Mock / BoNT)") + ylab("log2 (Ca / EGTA) - log2 (Mock / BoNT)")
  g <- g + annotate("text", y = 2.25, x = 1.2, color = "steelblue", label = lm_eqn(lm_mod), parse = TRUE, size = 7)
  g <- g + guides(color = guide_legend(override.aes = list(alpha = 1, size = 4), title = "Regulation Group"))
  ...
  pdf("Figures\\Fig_4A\\Fig_4A_mod.pdf", width = 10 , height = 7)
  ...
  g <- ggplot(ph_cand, aes(x = log2FC.BoNT, y = log2FC.CaEGTA, color = Regulation_group2))
  g <- g + geom_point(alpha = 0.6)
  g <- g + scale_color_manual(values = c("#ffa555", "#30a8ff", scales::alpha("grey", 0.4)))
  #g <- g + geom_abline(slope = lm_mod$coefficients[2], intercept = lm_mod$coefficients[1], color = "steelblue", linetype = "dashed")
  g <- g + xlab("log2 (Mock / BoNT)") + ylab("log2 (Ca / EGTA)")
  #g <- g + annotate("text", y = 2.25, x = 1.2, color = "steelblue", label = lm_eqn(lm_mod), parse = TRUE, size = 7)
  g <- g + guides(color = guide_legend(override.aes = list(alpha = 1, size = 4), title = "Regulation Group"))
