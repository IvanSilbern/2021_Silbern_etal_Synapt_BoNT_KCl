...
# input:
# "external\\data_Engholm-Keller_etal\\Phospho (STY)Sites.txt" table loaded as 'ph'
# "external\\data_Uniprot\\UP_AnnotationScore.tsv" containing uniprot scores
...
local({
  ...
  # read in fasta file
  fasta.file    <- fread("external\\data_Engholm-Keller_etal\\uniprot-proteome%3AUP000002494_rat_ref_proteome_plus_isoforms.fasta", sep = NULL, header = FALSE)[[1]]
  ...
  df <- fread( "external\\data_Engholm-Keller_etal\\Phospho (STY)Sites.txt", check.names = TRUE)
