...
local({
 ...
 if(!dir.exists("plots\\Lineplots_selected")) dir.create("plots\\Lineplots_selected", recursive = TRUE)
  ...
  for(i in seq_along(acc)){
    ...
    svglite(paste0("plots\\Lineplots_selected\\", df_caegta$Gene.name[df_caegta$Accession == acc[i]][1], "_", acc[i], "_CaEGTA.svg"), width = 6, height = 4)
  ...
  for(i in seq_along(acc)){
    ...
    svglite(paste0("plots\\Lineplots_selected\\", df_bont$Gene.name[df_bont$Accession == acc[i]][1], "_", acc[i], "_BoNT.svg"), width = 6, height = 4)
